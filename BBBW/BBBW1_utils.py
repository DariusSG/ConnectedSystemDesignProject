...
class OLED:
    ...
    def OLED_Display(self, text, fill=" ",coords=(0, 1)):
        ...
        true_x, true_y = (x * 8) + x_offset, (y * 8) + y_offset
        for line in text:
            self.Display.move(true_x, true_y).text(line[:18].center(23, fill), False)
            true_y += 8
        return True
    ...
    def ShowImage(self, path, offset_coords=(0,1)):
        ...
        self.loadImage(ImageObj, offset_coords)
...
class SSD1306OLED:
    ...
    SSD1306_MEMORYMODE = 0x20  # OK
    SSD1306_COLUMNADDR = 0x21  # OK
    SSD1306_PAGEADDR = 0x22  # OK
    SSD1306_WRITETOBUFFER = 0x40  # OK
    SSD1306_SETSTARTLINE = 0x40  # OK
    SSD1306_SETCONTRAST = 0x81  # OK
    SSD1306_CHARGEPUMP = 0x8D  # OK
    SSD1306_SEGREMAP = 0xA0  # OK
    SSD1306_DISPLAYALLON_RESUME = 0xA4  # OK
    SSD1306_DISPLAYALLON = 0xA5  # OK
    SSD1306_NORMALDISPLAY = 0xA6  # OK
    SSD1306_INVERTDISPLAY = 0xA7  # OK
    SSD1306_SETMULTIPLEX = 0xA8  # OK
    SSD1306_DISPLAYOFF = 0xAE  # OK
    SSD1306_DISPLAYON = 0xAF  # OK
    SSD1306_COMSCANDEC = 0xC8  # OK
    SSD1306_SETDISPLAYOFFSET = 0xD3  # OK
    SSD1306_SETDISPLAYCLOCKDIV = 0xD5  # OK
    SSD1306_SETPRECHARGE = 0xD9  # OK
    SSD1306_SETCOMPINS = 0xDA  # OK
    SSD1306_SETVCOMDETECT = 0xDB  # OK
    ...
    COS_TABLE = [
        0.000, 0.035, 0.070, 0.105, 0.140, 0.174, 0.208, 0.243, 0.276, 0.310, 0.343, 0.376, 0.408, 0.439, 0.471, 0.501,
        0.531, 0.561, 0.589, 0.617, 0.644,
        0.671, 0.696, 0.721, 0.745, 0.768, 0.790, 0.810, 0.830, 0.849, 0.867, 0.884, 0.900, 0.915, 0.928, 0.941, 0.952,
        0.962, 0.971, 0.979, 0.985, 0.991,
        0.995, 0.998, 1.000, 1.000, 0.999, 0.997, 0.994, 0.990, 0.984, 0.977, 0.969, 0.960, 0.949, 0.938, 0.925, 0.911,
        0.896, 0.880, 0.863, 0.845, 0.826,
        0.806, 0.784, 0.762, 0.739, 0.715, 0.690, 0.664, 0.638, 0.610, 0.582, 0.554, 0.524, 0.494, 0.463, 0.432, 0.400,
        0.368, 0.335, 0.302, 0.268, 0.234,
        0.200, 0.166, 0.131, 0.096, 0.062, 0.027, -0.008, -0.043, -0.078, -0.113, -0.148, -0.182, -0.217, -0.251,
        -0.284, -0.318, -0.351, -0.383, -0.415,
        -0.447, -0.478, -0.508, -0.538, -0.567, -0.596, -0.624, -0.651, -0.677, -0.702, -0.727, -0.750, -0.773, -0.795,
        -0.815, -0.835, -0.854, -0.872,
        -0.888, -0.904, -0.918, -0.931, -0.944, -0.955, -0.964, -0.973, -0.981, -0.987, -0.992, -0.996, -0.998, -1.000,
        -1.000, -0.999, -0.997, -0.993,
        -0.988, -0.982, -0.975, -0.967, -0.957, -0.947, -0.935, -0.922, -0.908, -0.893, -0.876, -0.859, -0.840, -0.821,
        -0.801, -0.779, -0.757, -0.733,
        -0.709, -0.684, -0.658, -0.631, -0.604, -0.575, -0.547, -0.517, -0.487, -0.456, -0.424, -0.392, -0.360, -0.327,
        -0.294, -0.260, -0.226, -0.192,
        -0.158, -0.123, -0.088, -0.053, -0.018]

    SIN_TABLE = [
        1.000, 0.999, 0.998, 0.994, 0.990, 0.985, 0.978, 0.970, 0.961, 0.951, 0.939, 0.927, 0.913, 0.898, 0.882, 0.865,
        0.847, 0.828, 0.808, 0.787,
        0.765, 0.742, 0.718, 0.693, 0.667, 0.641, 0.614, 0.586, 0.557, 0.528, 0.498, 0.467, 0.436, 0.404, 0.372, 0.339,
        0.306, 0.272, 0.238, 0.204,
        0.170, 0.135, 0.101, 0.066, 0.031, -0.004, -0.039, -0.074, -0.109, -0.144, -0.178, -0.213, -0.247, -0.280,
        -0.314, -0.347, -0.379, -0.412,
        -0.443, -0.474, -0.505, -0.535, -0.564, -0.593, -0.620, -0.647, -0.674, -0.699, -0.724, -0.747, -0.770, -0.792,
        -0.813, -0.833, -0.852,
        -0.870, -0.886, -0.902, -0.916, -0.930, -0.942, -0.953, -0.963, -0.972, -0.980, -0.986, -0.991, -0.995, -0.998,
        -1.000, -1.000, -0.999,
        -0.997, -0.994, -0.989, -0.983, -0.976, -0.968, -0.959, -0.948, -0.936, -0.924, -0.910, -0.895, -0.878, -0.861,
        -0.843, -0.823, -0.803,
        -0.782, -0.759, -0.736, -0.712, -0.687, -0.661, -0.635, -0.607, -0.579, -0.550, -0.520, -0.490, -0.459, -0.428,
        -0.396, -0.364, -0.331,
        -0.298, -0.264, -0.230, -0.196, -0.162, -0.127, -0.092, -0.057, -0.022, 0.013, 0.048, 0.083, 0.117, 0.152,
        0.187, 0.221, 0.255, 0.288,
        0.322, 0.355, 0.387, 0.419, 0.451, 0.482, 0.512, 0.542, 0.571, 0.599, 0.627, 0.654, 0.680, 0.705, 0.730, 0.753,
        0.776, 0.797, 0.818, 0.837,
    ...
    def __init__(self, reset_pin, i2c, address=0x3C, width=128, height=32):
        ...
        self.i2c.writeto(self.address,
                         bytes([0x00, self.SSD1306_SETCOMPINS, 0x02 if self.height in (16, 32) else 0x12]))
    ...
    def _coords_to_index(self, x, y):
        ...
        """
        x_list = list(reversed(range(self.width)))
        x = x_list[x]
